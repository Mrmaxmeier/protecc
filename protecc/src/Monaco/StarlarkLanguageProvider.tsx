...
function registerAutocomplete(monaco: Monaco) {
    function builtinVal(name: string, type: string, doc: string) {
        return {
            label: name,
    ...
    function builtinFun(name: string, args: string, retval: string, doc: string) {
        return {
            label: name,
    ...
    monaco.languages.registerCompletionItemProvider('starlark', {
        provideCompletionItems: () => {
            const tags = globalConfig === null ? null : Object.values(globalConfig.tags)
            const services = globalConfig === null ? null : Object.values(globalConfig.services)
...
If x is a string, it is interpreted as a sequence of digits in the specified base, decimal by default. If base is zero, x is interpreted like an integer literal, the base being inferred from an optional base marker such as 0b, 0o, or 0x preceding the first digit. These markers may also be used if base is the corresponding base. Irrespective of base, the string may start with an optional + or - sign indicating the sign of the result.`),
            ...
            ].concat(tags === null ? [] : tags.map((tag) => builtinVal('tag.' + tag.slug, 'bool', 'True if the tag ' + tag.name + ' is attached to this stream')))
                .concat(tags === null ? [] : tags.map((tag) => builtinVal('tags.' + tag.slug, 'int', 'The id of the tag ' + tag.name)))
                .concat(services === null ? [] : services.map((service) => builtinVal('service.' + service.slug, 'bool', 'True if this tag belongs to service ' + service.name)))
                .concat(services === null ? [] : services.map((service) => builtinVal('services.' + service.slug, 'int', 'The port of the service ' + service.name)));
            return { suggestions: suggestions } as any;
        }
    });
}

const starlarkTokensProvider = {
...
const starlarkLanguageDefinition = (monaco: Monaco) => ({
    comments: {
        lineComment: '#',
        blockComment: ['\'\'\'', '\'\'\''],
    },
